...
for x in range(X):
	for y in range(Y):
		if (y+4 < Y):
			maxi = max(int(grid[x][y])*int(grid[x][y+1])*int(grid[x][y+2])*int(grid[x][y+3]), maxi)
		if (x+4 < X):
			maxi = max(int(grid[x][y])*int(grid[x+1][y])*int(grid[x+2][y])*int(grid[x+3][y]), maxi)
		if (x > 2 and y+4 < Y):
			maxi = max(int(grid[x][y])*int(grid[x-1][y+1])*int(grid[x-2][y+2])*int(grid[x-3][y+3]), maxi)
		if (x+4 < X and y+4 < Y):
			maxi = max(int(grid[x][y])*int(grid[x+1][y+1])*int(grid[x+2][y+2])*int(grid[x+3][y+3]), maxi)
